...
with(t3, text(stem, vol, round(oArea - eArea,2)))
...
#overall
t3 <- with(me, aggregate(list(oVol=oVol, eVol=step1), list(vol=vol, stem=stem), FUN=sum))
tt <- matrix(0, nrow = max(me$stem), ncol = max(me$vol))
tt[cbind(t3$stem, t3$vol)] <- t3$oVol - t3$eVol
image(1:max(me$stem), 1:max(me$vol), tt, xlab="Stem Class", ylab="Volume Class", main="Observed - Estimated Volume")
with(t3, text(stem, vol, round(oVol - eVol)))
